...
| **Routing Layer** | `.agent/*/agent.md` | OpenAI, Gemini, etc. | One-liner pointers to corresponding CLAUDE.md |
...
### Step 6: Create .agent/ Routing Structure

Create the `.agent/` directory mirroring the CLAUDE.md structure:

1. Create `.agent/` folder at project root
2. For each directory with a CLAUDE.md, create a corresponding `.agent/[directory]/agent.md`
3. Each `agent.md` contains only a **single routing line**:

```md
Read and follow all instructions in `../../[directory]/CLAUDE.md`.
```

Example for `.agent/frontend/agent.md`:

```md
Read and follow all instructions in `../../frontend/CLAUDE.md`.
```

### Step 7: Create .agent/index.md

Create a hub file at `.agent/index.md` listing all agents:

```md
# Agents Index

| Agent | Path | Routes To |
|-------|------|-----------|
| frontend | `.agent/frontend/agent.md` | `frontend/CLAUDE.md` |
| backend | `.agent/backend/agent.md` | `backend/CLAUDE.md` |
| components | `.agent/components/agent.md` | `components/CLAUDE.md` |
| database | `.agent/database/agent.md` | `database/CLAUDE.md` |
