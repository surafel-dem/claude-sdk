...
* Argument: --template <template>
  * Description: Select a starter template and skip the interactive template prompt. Templates may include names like bun, cloudflare-workers, vercel, etc.
  * Example: --template cloudflare-workers
* Argument: --install
  * Description: Automatically install dependencies after the template is created.
  * Example: --install
* Argument: --pm <packageManager>
  * Description: Specify which package manager to run when installing dependencies. Common values: npm, pnpm, yarn.
  * Example: --pm pnpm
* Argument: --offline
  * Description: Use the local cache/templates instead of fetching the latest remote templates. Useful for offline environments or deterministic local runs.
  * Example: --offline
